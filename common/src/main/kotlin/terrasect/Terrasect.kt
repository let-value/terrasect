...
object Terrasect {
  ...
  fun init() {
    ...
    registry.region("hex").radius(150).strategy(Strategy.hex().tiling(true))
    registry.region("cell").parent("hex").strategy(Strategy.voronoi())

    registry.region("voronoi1").radius(30).parent("cell").climate {
      temperature(-10000).humidity(5000)
    }
    registry.region("voronoi2").radius(45).parent("cell").climate { temperature(0).humidity(0) }
    registry.region("voronoi3").radius(75).parent("cell").climate {
      temperature(10000).humidity(-5000)
    }

    registry.region("border").radius(500)
