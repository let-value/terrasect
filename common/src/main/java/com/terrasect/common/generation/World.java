...
public final class World {
  ...
  public static void initialize(Context context) {
    ...
    if (anchored != null) {
      ...
      Terrasect.LOGGER.info(
          "Grid offset for '{}': dx={}, dz={}", dimensionId, offset[0], offset[1]);
    } else {
  ...
  public static Region emptyRoot(String name) {
    return new Region(
        name, 10000, RegionDefinition.empty(), Collections.emptySet(), List.of(), List.of(), false);
  }
  ...
  private static float[] calculateAnchorOffset(
      Region root, Region anchored, long seed, Context context) {
    ...
    for (var ring = 1; ring <= maxRings; ring++) {
      ...
      for (var i = 0; i < pointsPerRing; i++) {
        ...
        if (sampled.region != null && sampled.region.name().equals(targetName)) {
          ...
          Terrasect.LOGGER.info(
              "Found anchored region '{}' at ({}, {})", targetName, sampleX, sampleZ);
    ...
    Terrasect.LOGGER.warn(
        "Could not find anchored region '{}' within {} rings", targetName, maxRings);
    return new float[] {0, 0};
  }
