...
public final class ClusterMapGenerator {
    ...
    private static final double BASIN_STEP_RATIO = 0.78;
    private static final double BASIN_STEP_FALLOFF = 0.65;
    private static final double BASIN_PULL = 0.9;
    private static final double BASIN_FIELD_SCALE = 0.02;
    private static final int BASIN_RELAX_STEPS = 5;
    ...
    int resolveRegion(int x, int y, int clusterSize, ClusterSite site,
    ...
    ClusterSite locateSite(int clusterSize, long seed, int x, int y) {
    ...
    private double basinNoise(long seed, int clusterSize, double x, double y) {
        ...
        double nx = x * scale;
        double ny = y * scale;
        double ridge = 1.0 - Math.abs(fbm(seed + 211, nx * 1.1, ny * 1.1, 2) * 2.0 - 1.0);
        double base = fbm(seed + 389, nx * 0.65, ny * 0.65, 3);
        double flow = fbm(seed - 947, nx * 0.45, ny * 0.45, 2);
        return base * 0.6 + ridge * 0.2 + flow * 0.2;
    }

    ResolvedSite resolveSite(ClusterSite site, List<RegionDefinition> regions, int clusterSize) {
