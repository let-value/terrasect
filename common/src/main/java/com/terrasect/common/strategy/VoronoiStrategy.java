...
public final class VoronoiStrategy {
  ...
  private static final ThreadLocal<float[]> RADII_BUFFER =
      ThreadLocal.withInitial(() -> new float[8]);
  private static final ThreadLocal<float[]> SITES_BUFFER =
  ...
  private static void computeRelaxedSites(
      long seed, int count, float[] radii, int iterations, float[] sites) {
