...
public final class TemplateStrategy {
  ...
  public static void query(
    ...
    int centerIndex =
        (type == TemplateType.CENTER_SURROUND)
            ? findCenterIndex(children, centerRegionName, totalBudget)
            : 0;
  ...
  private static TemplateType selectTemplate(
      GenerationStrategy.TemplateType explicit,
      List<Region> children,
      float totalBudget,
      int count) {
    if (explicit != null) {
  ...
  private static int findCenterIndex(
      List<Region> children, String centerRegionName, float totalBudget) {
