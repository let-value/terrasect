...
class MinecraftEdgeSamplerTest {
  ...
  void collectsMultiScaleEdgeStatisticsFromOverworldSlice() {
    ...
    var parameterListLookup =
        lookup.lookupOrThrow(Registries.MULTI_NOISE_BIOME_SOURCE_PARAMETER_LIST);
    var overworldParameters =
        parameterListLookup.getOrThrow(
    ...
    System.out.printf(
        "Coarse: density=%.4f jitter=(h=%.2f,v=%.2f) avg run blocks=%.2f%n",
    ...
    assertTrue(
        fineStatistics.distinctBiomes() >= 8, "expected a diverse biome mix in overworld slice");
    assertTrue(
        fineStatistics.transitionTotal() > 0,
        "should have plenty of biome transitions at chunk scale");
    assertTrue(
        fineStatistics.averageRunLength() > 1.5 && fineStatistics.averageRunLength() < 64,
        "run lengths should capture small and mid-scale striping");

    assertTrue(
        fineStatistics.roughness().transitionDensity() > 0.02,
        "biome noise should produce frequent edge crossings");
    assertTrue(
        fineStatistics.roughness().meanHorizontalJitter() < 48,
        "edge jitter should be bounded at block-scale detail");

    assertTrue(
        coarseStatistics.roughness().transitionDensity() > 0.005,
        "coarse sampling should still capture macro transitions");
    assertTrue(
        coarseStatistics.averageRunLengthBlocks() >= fineStatistics.averageRunLengthBlocks(),
        "coarser sampling should yield longer stretches in world distance");
    assertTrue(
        coarseStatistics.roughness().meanVerticalJitter() >= 0,
        "jitter metric should remain non-negative at all scales");

    assertTrue(
        fineStatistics.climateEdgeDeltas().hasSamples(),
        "should have climate deltas recorded along edges");
    assertTrue(
        fineStatistics.climateEdgeDeltas().averageTemperatureDelta() > 0.01,
    ...
    if (fineStatistics.biomeCounts().containsKey(riverKey)) {
      assertTrue(
          fineStatistics.transitionCounts().keySet().stream()
              .anyMatch(key -> key.contains(riverKey)),
