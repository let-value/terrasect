...
public final class SnapshotHtmlReports {
    ...
    public static void writeIndex(File outDir, String title, List<ImageEntry> images) throws IOException {
    ...
    public static void writeIndexWithLegend(
            File outDir, String title, List<ImageEntry> images, List<LegendEntry> legend) throws IOException {
        var hasLegend = legend != null && !legend.isEmpty();
        var html = renderTemplate(
                "gallery/with-legend.mustache", new LegendGalleryPage(title, images, legend, hasLegend));
        Files.writeString(outDir.toPath().resolve("index.html"), html, StandardCharsets.UTF_8);
    }
