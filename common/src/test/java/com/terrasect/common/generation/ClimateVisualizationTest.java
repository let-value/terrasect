  ...
  public void visualizeClimateInfluence(Snapshot snapshot) throws IOException {
    ...
    System.out.println(
        "Modification coverage: "
            + String.format("%.1f%%", 100.0 * totalModified / (WIDTH * HEIGHT)));
    if (totalModified > 0) {
      System.out.println(
          "Average temperature delta: " + String.format("%.1f", totalTempDelta / totalModified));
      System.out.println(
          "Average humidity delta: " + String.format("%.1f", totalHumidityDelta / totalModified));
    }
    System.out.println(
        "Biomes changed by climate: "
            + biomesChanged
            + " ("
    ...
    snapshotText
        .append("totalTempDelta=")
        .append(String.format(Locale.ROOT, "%.4f", totalTempDelta))
        .append('\n');
    snapshotText
        .append("totalHumidityDelta=")
        .append(String.format(Locale.ROOT, "%.4f", totalHumidityDelta))
        .append('\n');
            ...
            regions ->
                regions
                    ...
                    .child(
                        "BURNING_WASTES",
                        region ->
                            region.radius(500).climate(c -> c.temperature(1.0f).humidity(0.0f)))
                    .child(
                        "FROZEN_NORTH",
                        region ->
                            region.radius(100).climate(c -> c.temperature(0.0f).humidity(0.3f)))
                    .child(
                        "JUNGLE_HEART",
                        region ->
                            region.radius(200).climate(c -> c.temperature(0.9f).humidity(1.0f)))
                    .child(
                        "VERDANT_WOODS",
                        region ->
                            region.radius(300).climate(c -> c.temperature(0.5f).humidity(0.6f)))
                    .child("UNTOUCHED_LANDS", region -> region.radius(100)));

    return registry.build("WORLD");
  }

  private Context createStrategy(
      long seed, Climate.Sampler sampler, MultiNoiseBiomeSource biomeSource) {
    return new Context() {
      @Override
      public long getSeed() {
        return seed;
