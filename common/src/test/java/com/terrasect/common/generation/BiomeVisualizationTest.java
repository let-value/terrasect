...
public class BiomeVisualizationTest {
  ...
  public void visualizeBiomeFiltering(Snapshot snapshot) throws IOException {
    ...
    for (var py = 0; py < HEIGHT; py++) {
      for (var px = 0; px < WIDTH; px++) {
        ...
        Set<String> vanillaTags =
            vanillaEntry != null ? vanillaEntry.tags() : getBiomeTags(vanillaBiome);
    ...
    SnapshotHtmlReports.writeIndex(
        ...
        List.of(
            ...
            SnapshotHtmlReports.ImageEntry.of(
                "filtered_biomes", "filtered_biomes.png", WIDTH, HEIGHT),
            SnapshotHtmlReports.ImageEntry.of(
                "filter_overlay", "filter_overlay.png", WIDTH, HEIGHT),
    ...
    System.out.println(
        "Block rate (of filtered area): "
            + String.format(
                "%.2f%%", totalWithRules > 0 ? 100.0 * blockedCount / totalWithRules : 0));
    System.out.println(
        "Biomes changed: "
            + replacedCount
            + " ("
            + String.format("%.1f%%", 100.0 * replacedCount / (WIDTH * HEIGHT))
            + " of total)");
  ...
  private Region buildFilteredBiomeRegions() {
    ...
    registry
        ...
        .child(
            "ETERNAL_FOREST",
            region -> region.radius(150).biomes(b -> b.allowTags("#minecraft:is_forest")))
        .child(
            "LANDLOCKED",
            region ->
                region
  ...
  private long calculateClimateDistance(Climate.TargetPoint target, Climate.ParameterPoint params) {
    ...
    var contDist = Math.abs(target.continentalness() - params.continentalness().min());
