...
public class StrategySnapshotTest {
  ...
  void template_binary(Snapshot snapshot) throws Exception {
    var root =
        buildBinaryHierarchy(GenerationStrategy.template(GenerationStrategy.TemplateType.BINARY));
  ...
  void template_triangle(Snapshot snapshot) throws Exception {
    var root =
        buildTriangleHierarchy(
            GenerationStrategy.template(GenerationStrategy.TemplateType.TRIANGLE));
  ...
  void template_center_surround_named(Snapshot snapshot) throws Exception {
    ...
    var root =
        buildCenterSurroundHierarchy(
            GenerationStrategy.centerSurround("SMALL_CENTER"), "SMALL_CENTER");
  ...
  void template_radial(Snapshot snapshot) throws Exception {
    var root =
        buildRadialHierarchy(GenerationStrategy.template(GenerationStrategy.TemplateType.RADIAL));
  ...
  private void runSnapshotTest(Snapshot snapshot, String testName, Region root, int maxDepth)
      throws Exception {
  ...
  private Region buildCenterSurroundHierarchy(
      GenerationStrategy templateStrategy, String centerName) {
    var registry = new RegionRegistry();
    registry
        .region("WORLD")
        .strategy(GenerationStrategy.hex())
                            ...
                            city.radius(250)
                                .strategy(
                                    GenerationStrategy.template(
                                        GenerationStrategy.TemplateType.RADIAL))
  ...
  private Region buildKitchenSink() {
    ...
    registry
        ...
        .child(
            "LANDMASS",
            land ->
                land.strategy(GenerationStrategy.voronoi())
                    .child(
                        "KINGDOM",
                        kingdom ->
                            kingdom
  ...
  private String runSnapshot(String testName, Region root, int maxDepth) throws Exception {
    ...
    for (var d = 1; d <= maxDepth; d++) {
      images.add(
          SnapshotHtmlReports.ImageEntry.of(
              "depth_" + d, "depth_" + d + ".png", IMG_SIZE, IMG_SIZE));
    }
    ...
    SnapshotHtmlReports.writeIndexWithLegend(
        outDir, "Strategy Snapshot: " + testName, images, legendEntries);
  ...
  private int getRegionColor(String name) {
    ...
    return switch (name) {
      ...
      case "VILLAGE_NORTH", "VILLAGE_SOUTH", "VILLAGE_N", "VILLAGE_E", "VILLAGE_S", "VILLAGE_W" ->
          0xDEB887;
